...
$(function () {
  ...
  (function init() {
    ...
    function generate() {
      ...
      let forceNumCheckBox = $('#forceNumCheckBox').prop('checked');
      ...
      let getIterateLines = (arr, className, key, legalKey, jsonKey, shouldNullSafe) => {
        ...
        if (typeof inner === 'object') {
          ...
          if (typeof inner === 'boolean') {
            ...
            if (typeof inner === 'number') {
              if (forceNumCheckBox) {
                toType = shouldEnhanceFaultTolerance ? 'num.tryParse(v.toString() ?? \'\')' : 'v.toInt()';
              } else {
                if (Number.isInteger(inner)) {
            ...
            if (typeof element === 'object') {
              ...
              if (typeof element === 'boolean') {
                ...
                if (typeof element === 'string') {
                  ...
                  if (forceNumCheckBox) {
                    toType = shouldEnhanceFaultTolerance ? `num.tryParse(json[${jsonKey}]?.toString() ?? '')` : `json[${jsonKey}]?.toInt()`;
                      type = 'num';
                  } else {
      ...
      let dartCode = `///\n///${date.getFullYear()}-${date.getMonth() + 1}-${date.getDay()} ${date.toLocaleDateString()}\n///\n///\n${objToDart(jsonObj, rootClass, "")}`;
